...
type EntryOptions struct {
	...
	// fromService 来源服务，如果为空或者为default则意味着没有设置
	fromService string
}
...
func WithFromService(fromService string) EntryOption {
	return func(options *EntryOptions) {
		options.fromService = fromService
	}
}
...
func entry(resource string, options *EntryOptions) (*base.SeaEntry, *base.BlockError) {
	...
	ctx.FromService = options.fromService
	...
	if len(options.cookies) != 0 {
		ctx.Input.Cookies = options.cookies
	}
	if len(options.body) != 0 {
		ctx.Input.Body = options.body
	}
	...
	e.SetGrayResource(r.GrayRes())
	e.SetGrayTag(r.GrayTag())
	e.SetLinkPass(r.LinkPass())
	e.SetGrayAddress(r.GrayAddress())
	return e, nil
