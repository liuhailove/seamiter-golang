...
var (
	...
	nopStat      = &standaloneStatistic{
		...
		writeOnlyMetric:   base.NoWriteStat(),
	}
...
func generateStatFor(rule *Rule) (*standaloneStatistic, error) {
	...
	err := base.CheckValidityForReuseStatistic(sampleCount, intervalInMs, config.GlobalStatisticSampleCountTotal(), config.MetricStatisticIntervalMs())
	if err == nil {
