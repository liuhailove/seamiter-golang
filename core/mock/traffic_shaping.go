...
var (
	...
	randFunc2LenMap = make(map[string]int)
)

func init() {
	randFunc2LenMap[RandStr1] = 1
	randFunc2LenMap[RandStr2] = 2
	randFunc2LenMap[RandStr3] = 3
	randFunc2LenMap[RandStr4] = 4
	randFunc2LenMap[RandStr5] = 5
	randFunc2LenMap[RandStr6] = 6
	randFunc2LenMap[RandStr7] = 7
	randFunc2LenMap[RandStr8] = 8
	randFunc2LenMap[RandStr9] = 9
	randFunc2LenMap[RandStr10] = 10
	randFunc2LenMap[RandStr11] = 11
	randFunc2LenMap[RandStr12] = 12
	randFunc2LenMap[RandStr13] = 13
	randFunc2LenMap[RandStr14] = 14
	randFunc2LenMap[RandStr15] = 15
	randFunc2LenMap[RandStr16] = 16
	randFunc2LenMap[RandStr17] = 17
	randFunc2LenMap[RandStr18] = 18
	randFunc2LenMap[RandStr19] = 19
	randFunc2LenMap[RandStr20] = 20
...
func (c *baseTrafficShapingController) ArgsCheck(ctx *base.EntryContext) *RuleItem {
	...
	if ctx.Resource.Classification() == base.ResTypeMicro {
		var requestJsonData []byte = nil
		var err error
		var args0 = args[0]
		if structs.IsStruct(args0) {
			if requestJsonData, err = jsonTraffic.Marshal(args[0]); err != nil {
				logging.Warn("request cannot transfer to json", "err", err)
			}
		} else if requestJsonDataStr, ok := args0.([]byte); ok {
			requestJsonData = requestJsonDataStr
		}
		// 模式匹配
		for _, item := range c.specificItems {
			...
			// 随机方法替换
			for funK, l := range randFunc2LenMap {
				// 随机函数替换
				item.ThenReturnMockData = strings.ReplaceAll(item.ThenReturnMockData, funK, util.RandStr(l))
			}
			// 处理请求、响应替换
...
func (c *baseTrafficShapingController) getPropertyValue(paramKey string, requestJsonData []byte) (string, error) {
	...
	var findObj, _, _, err = jsonparser.Get(requestJsonData, propertyArr2...)
	return string(findObj), err
}
...
func (c *baseTrafficShapingController) kindStringHandle(requestJsonData []byte, propertyArr []string, item RuleItem, ctx *base.EntryContext) *RuleItem {
	var val string
	var err error
	if item.WhenParamSource == ParameterTypeUnknown || item.WhenParamSource == ParameterTypeParameter {
		// 参数类型为"参数"或者"未知"的处理逻辑，由于此参数为后来加的，因此未知是为了兼容老数据
		val, err = jsonparser.GetString(requestJsonData, propertyArr...)
	} else if item.WhenParamSource == ParameterTypeMetadata {
		// 参数类型为"Metadata"的取数逻辑
		val = ctx.Input.MetaData[item.WhenParamKey]
	}
	if err != nil {
		logging.Warn("get property failed", "paramSource", item.WhenParamSource.String(), "property", item.WhenParamKey, "request data", requestJsonData, "err", err)
		return nil
	}
	var changeData []byte
	// 响应替换
...
func (c *baseTrafficShapingController) generalHandle(requestJsonData []byte, propertyArr []string, item RuleItem, ctx *base.EntryContext) *RuleItem {
	...
	if item.WhenParamSource == ParameterTypeUnknown || item.WhenParamSource == ParameterTypeParameter {
		...
		val = []byte(ctx.Input.MetaData[item.WhenParamKey])
		// 参数类型为"Metadata"的取数逻辑
		if item.WhenParamKind == KindNumber {
			// 特殊处理，此处是指强制把Metadata的String转为数子
			dt = jsonparser.Number
		} else {
			// MataData的value类型能为String
	...
	if item.MockReplace == Resp {
		var valS string
		if dt == jsonparser.Array || dt == jsonparser.Boolean || dt == jsonparser.Number {
