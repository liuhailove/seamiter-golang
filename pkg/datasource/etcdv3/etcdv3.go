...
func (s *Etcdv3DataSource) Write(bytes []byte) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	_, err := s.client.Put(ctx, s.propertyKey, string(bytes))
	if err != nil {
		logging.Error(err, "Fail to write data for key when execute Etcdv3DataSource.Write()", "propertyKey", s.propertyKey)
		return err
	}
	return nil
}
...
func NewDataSource(client *clientv3.Client, key string, handlers ...datasource.PropertyHandler) (*Etcdv3DataSource, error) {
	...
	ds := &Etcdv3DataSource{client: client, propertyKey: key}
...
func (s *Etcdv3DataSource) ReadSource() ([]byte, error) {
	...
	s.lastUpdateRevision = resp.Header.Revision
	logging.Info("[Etcdv3] Get the newest data for key", "propertyKey", s.propertyKey,
		"revision", resp.Header.Revision, "value", resp.Kvs[0].Value)
...
func (s *Etcdv3DataSource) processWatchResponse(resp *clientv3.WatchResponse) {
	if resp.CompactRevision > s.lastUpdateRevision {
		s.lastUpdateRevision = resp.CompactRevision
	}
	if resp.Header.Revision > s.lastUpdateRevision {
		s.lastUpdateRevision = resp.Header.Revision
	}
...
func (s *Etcdv3DataSource) watch() {
	...
	rch := s.client.Watch(ctx, s.propertyKey, clientv3.WithCreatedNotify(), clientv3.WithRev(s.lastUpdateRevision))
	for {
		for resp := range rch {
		...
		ctx, cancel = context.WithCancel(context.Background())
		s.cancel = cancel
		if s.lastUpdateRevision > 0 {
			rch = s.client.Watch(ctx, s.propertyKey, clientv3.WithRev(s.lastUpdateRevision+1))
		} else {
