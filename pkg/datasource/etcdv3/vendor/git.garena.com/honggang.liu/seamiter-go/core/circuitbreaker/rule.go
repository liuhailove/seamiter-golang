...
type Rule struct {
	...
	// TimeWindow represents recovery timeout (in milliseconds) before the circuit breaker opens.
	...
	TimeWindow uint32 `json:"timeWindow"`
...
func (r *Rule) String() string {
	// fallback string
	return fmt.Sprintf("{id=%s, resource=%s, strategy=%s, TimeWindow=%d, MinRequestAmount=%d, StatIntervalMs=%d, StatSlidingWindowBucketCount=%d, MaxAllowedRtMs=%d, Count=%f}",
		r.Id, r.Resource, r.Strategy, r.TimeWindow, r.MinRequestAmount, r.StatIntervalMs, r.StatSlidingWindowBucketCount, r.MaxAllowedRtMs, r.Threshold)
}
...
func (r *Rule) isStatReusable(newRule *Rule) bool {
	...
	return r.Resource == newRule.Resource && r.Strategy == newRule.Strategy &&
		r.StatIntervalMs == newRule.StatIntervalMs && r.StatSlidingWindowBucketCount == newRule.StatSlidingWindowBucketCount
}
...
func (r *Rule) isEqualsToBase(newRule *Rule) bool {
	...
	return r.Resource == newRule.Resource && r.Strategy == newRule.Strategy && r.TimeWindow == newRule.TimeWindow &&
