...
const (
	...
	KindByte              = "byte"
	KindArr               = "array"
	KindNumber            = "number" // 数值类型，涵盖[int,int32,int64,float32,float64]
)
...
const (
	...
	ParameterTypeCookie    ParameterSourceType = 1
	ParameterTypeHeader    ParameterSourceType = 2
	ParameterTypeParameter ParameterSourceType = 3
	ParameterTypeBody      ParameterSourceType = 4
	ParameterTypeMetadata  ParameterSourceType = 5
)
...
type RuleItem struct {
	// 参数来源
	WhenParamSource ParameterSourceType `json:"whenParamSource"`
	// 参数索引
	...
	// ThenReturnWaitingTimeMs 等待时间，当ReplaceMode==ctx超时时间时，此处表示的时ctx的超时
	ThenReturnWaitingTimeMs int64 `json:"thenReturnWaitingTimeMs"`
	// mock数据
	ThenThrowMsg string `json:"thenThrowMsg"`
	// mockReplace mock替换，开启则WhenParamValue不起作用，0不替换，1替换响应，2替换请求，3ctx超时时间
	...
	// 新增，为了支持多参数匹配的mock
	// 参数关系
	ParamOP Op `json:"paramOP"`
	// 参数名称，优先级大于参数索引
	WhenParamKey2 string `json:"whenParamKey2"`
	// 参数值
	WhenParamValue2 string `json:"whenParamValue2"`
	// 参数类型
	WhenParamKind2 ParamKind `json:"whenParamKind2"`
	// 参数名称，优先级大于参数索引
	WhenParamKey3 string `json:"whenParamKey3"`
	// 参数值
	WhenParamValue3 string `json:"whenParamValue3"`
	// 参数类型
	WhenParamKind3 ParamKind `json:"whenParamKind3"`

	TmpData interface{} `json:"-"`
}

func (r *RuleItem) isEqualTo(newRuleItem *RuleItem) bool {
	return r.WhenParamIdx == newRuleItem.WhenParamIdx && r.WhenParamKey == newRuleItem.WhenParamKey && r.WhenParamValue == newRuleItem.WhenParamValue && r.WhenParamKind == newRuleItem.WhenParamKind &&
		...
		r.AdditionalItemKey == newRuleItem.AdditionalItemValue && r.AdditionalItemValue == newRuleItem.AdditionalItemValue && r.ParamOP == newRuleItem.ParamOP &&
		r.WhenParamKey2 == newRuleItem.WhenParamKey2 && r.WhenParamValue2 == newRuleItem.WhenParamValue2 && r.WhenParamKind2 == newRuleItem.WhenParamKind2 &&
		r.WhenParamKey3 == newRuleItem.WhenParamKey3 && r.WhenParamValue3 == newRuleItem.WhenParamValue3 && r.WhenParamKind3 == newRuleItem.WhenParamKind3 && r.WhenParamSource == newRuleItem.WhenParamSource
...
func (r *Rule) isEqualsToBase(newRule *Rule) bool {
	...
	var baseEqual = r.Resource == newRule.Resource && r.ControlBehavior == newRule.ControlBehavior && r.Strategy == newRule.Strategy && r.ThenReturnMockData == newRule.ThenReturnMockData && r.ThenThrowMsg == newRule.ThenThrowMsg && r.RequestHold == newRule.RequestHold && r.LimitApp == newRule.LimitApp
	if !baseEqual {
		return false
	}
