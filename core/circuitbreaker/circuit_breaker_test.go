...
func TestSlowRtCircuitBreaker_TryPass(t *testing.T) {
	...
	t.Run("TryPass_Probe", func(t *testing.T) {
		...
		ctx := &base.EntryContext{Resource: base.NewResourceWrapper("abc", base.ResTypeCommon, base.Inbound)}
		e := base.NewSeaEntry(ctx, base.NewResourceWrapper("abc", base.ResTypeCommon, base.Inbound), nil)
		ctx.SetEntry(e)
		pass := b.TryPass(ctx)
		assert.True(t, pass)
...
func TestSlowRtCircuitBreaker_OnRequestComplete(t *testing.T) {
	ClearStateChangeListeners()
	r := &Rule{
		Resource:         "abc",
		Strategy:         SlowRequestRatio,
	...
	b, err := newSlowRtCircuitBreaker(r)
	assert.Nil(t, err)
	t.Run("OnRequestComplete_Less_Than_MinRequest", func(t *testing.T) {
...
func TestErrorCountCircuitBreaker_OnRequestComplete(t *testing.T) {
	r := &Rule{
		Resource:         "abc",
		Strategy:         ErrorCount,
		RetryTimeoutMs:   3000,
