...
type Item struct {
	...
	// 附加参数Values，多个值，值之间逗号分割
	AdditionalItemValues []string `json:"additionalItemValues"`
}
...
type Rule struct {
	...
	// **响应结构体**
	// SpecificItems 指定一些特定的响应属性的重试规则
	SpecificItems []Item `json:"specificItems"`
}
...
func (r *Rule) String() string {
	...
	return fmt.Sprintf("{id=%s, resource=%s, retryPolicy=%s,retryMaxAttempts=%d,retryTimeout=%d,"+
		...
		"errorMatcher=%s,includeExceptions=%s,excludeExceptions=%s,item=%s}",
		...
		r.ErrorMatcher.String(), r.IncludeExceptions, r.ExcludeExceptions, r.SpecificItems)
}
...
func (r *Rule) isStatReusable(newRule *Rule) bool {
	...
	for i := 0; i < len(r.SpecificItems); i++ {
		if r.SpecificItems[i].AdditionalItemKey != newRule.SpecificItems[i].AdditionalItemKey {
			return false
		}
		if len(r.SpecificItems[i].AdditionalItemValues) != len(newRule.SpecificItems[i].AdditionalItemValues) {
			return false
		}
		for j := 0; j < len(r.SpecificItems[i].AdditionalItemValues); j++ {
			if r.SpecificItems[i].AdditionalItemValues[j] != newRule.SpecificItems[i].AdditionalItemValues[j] {
				return false
			}
		}
	}
