...
import (
	...
	cb "github.com/liuhailove/seamiter-golang/core/circuitbreaker"
	...
	retry "github.com/liuhailove/seamiter-golang/core/retry/rule"
...
func GrayRuleJsonArrayParser(src []byte) (interface{}, error) {
	...
	var json = jsoniter.ConfigCompatibleWithStandardLibrary
	if err := json.Unmarshal(src, &rules); err != nil {
		desc := fmt.Sprintf("Fail to convert source bytes to []*gray.Rule, err: %s", err.Error())
		return nil, NewError(ConvertSourceError, desc)
	}
	return rules, nil
}
...
func GrayRulesUpdater(data interface{}) error {
	...
	if val, ok := data.([]gray.Rule); ok {
		...
		return NewError(UpdatePropertyError, fmt.Sprintf("Fail to type assert data to []gray.Rule or []*gray.Rule, in fact, data: %+v", data))
	}
