...
// BlockError indicates the request was blocked by sea.
...
func WithRule(rule SeaRule) BlockErrorOption {
	return func(b *BlockError) {
		b.rule = rule
	}
}
...
func NewBlockError(opts ...BlockErrorOption) *BlockError {
	b := &BlockError{blockType: BlockTypeUnknown}
...
func (e *BlockError) TriggeredRule() SeaRule {
	return e.rule
}
...
func NewBlockErrorWithCause(blockType BlockType, blockMsg string, rule SeaRule, snapshot interface{}) *BlockError {
...
func (e *BlockError) Error() string {
	...
	if len(e.blockMsg) == 0 {
		return fmt.Sprintf("seaBlockError: %s", e.blockType.String())
	}
	return fmt.Sprintf("seaBlockError: %s, message: %s", e.blockType.String(), e.blockMsg)
}
